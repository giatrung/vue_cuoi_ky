<template>
  <div>
    <h1 class="text-center">Đơn hàng của thành viên</h1>
        ...
        <tr v-for="(item, index) in orders" :key="index" >
            ...
            <td @click="showInfo(index)">{{ item.id }} 
              <table class="table tableShowOrderDetail" v-if="indexToShow == index">
                ...
                <tbody>
                  <tr>
                    <td rowspan="20">{{userInfoToShow.data.name}}</td>
                    <td rowspan="20">{{userInfoToShow.data.phone}}</td>
                    <td rowspan="20">{{userInfoToShow.data.address}}</td>
                  </tr>
    ...
    showInfo(index){
      if(this.indexToShow == index){
        ...
        let userIndex = this.users.findIndex(el =>{
          return el.id == this.orders[index].data.userId
        });
        // console.log("Orders", this.orders[index].data.userId);
        this.userInfoToShow = this.users[userIndex]
